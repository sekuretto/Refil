...
function wppb_login_form( $args = array() ) {
	...
	$defaults = array(
		...
		'redirect' => ( is_ssl() ? 'https://' : 'http://' . $_SERVER['HTTP_HOST'] . '/index.php/ohjauspaneeli/' ),
		'form_id' => 'loginform',
		'label_username' => __( 'Sähköposti' ),
		'label_password' => __( 'Salasana' ),
		'label_remember' => __( 'Muista minut' ),
		'label_log_in' => __( 'Kirjaudu sisään' ),
...
function wppb_front_end_login( $atts ){
	...
	if( !is_user_logged_in() ){
		...
		if ( isset( $wppb_generalSettings['loginWith'] ) && ( $wppb_generalSettings['loginWith'] == 'email' ) )
			$form_args['label_username'] = __( 'Sähköposti', 'profile-builder' );
        ...
        $logout_url = '<a href="'.wp_logout_url( $logout_redirect_url ).'" class="wppb-logout-url" title="'.__( 'Log out of this account', 'profile-builder' ).'">'. __( 'Kirjaudu ulos', 'profile-builder').' &raquo;</a>';
		$logged_in_message .= sprintf(__( 'Olet kirjautunut sisään käyttäjällä %1$s. %2$s', 'profile-builder' ), $display_name, $logout_url );
