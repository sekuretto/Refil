 ...
 */
if (!class_exists('CBXWPSimpleaccounting_Settings_API')):

    class CBXWPSimpleaccounting_Settings_API {
	    ...
	    function getMissingDefaultValueBySection($section_id){
		    ...
		    foreach ($fields as $field){
		        if(!isset($section_value[$field['name']])){
			        $section_value[$field['name']] =  $field['default'];
                }
        ...
        function callback_text($args) {
            ...
            $html = sprintf('<input '.$step.' type="%1$s" class="%2$s-text" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"/>', $type, $size, $args['section'], $args['id'], $value);
        ...
        function show_forms() {
            ...
            <div class="metabox-holder">
                <?php foreach ($this->settings_sections as $form) {  ?>
                    <div id="<?php echo $form['id']; ?>" class="cbxwpsimpleaccounting_group" style="display: none;">
                        <form method="post" action="options.php">
                            <?php
                            do_action('cbxwpsimpleaccounting_setting_form_top_' . $form['id'], $form);
                            settings_fields($form['id']);
                            do_settings_sections($form['id']);
                            do_action('cbxwpsimpleaccounting_setting_form_bottom_' . $form['id'], $form);
